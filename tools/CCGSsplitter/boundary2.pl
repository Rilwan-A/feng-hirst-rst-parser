...
{
   $opt_h = 1;
   print "Usage: boundary.pl -d honorifics_file -i str_utterance\n";
   exit;
}
...
if ($opt_i eq "")
{
   print "$opt_i is an empty string!\n";
   exit;
}
...
# This subroutine checks if the string is a standard representation of a popular
# timezone
sub IsTimeZone
{
   my($word) = @_;
   ...
   if ($word eq "EDT" || $word eq "CST" || $word eq "EST" || $word eq "GMT")
   {
      return 1;
   }
   else
